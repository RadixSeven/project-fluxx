...
The DAG panel has two modes: DAG display (which shows as a graph with collapsable nodes) and the list display. A radio button control in the control bar above the DAG display switches between the two modes. The list mode displays list entries with the node title and has a search bar to narrow the list by substrings. When you've clicked on a node in the list view, it switches back to the DAG view with the new node panned to the center.
...
You can click on a dependency in the editor pane to edit that dependency. It switches to the edit-dependency mode described above. There is also a clickable trashcan 🗑️ to remove them.
...
There is also a button on the DAG button bar to see the simulations associated with the current DAG. This opens in the edit panel. It displays a list of simulations with a button to create a new simulation. When you create a simulation, it opens a dialog to collect the parameters. While a simulation is running, there is a progress bar with elapsed time, an estimated time to completion, currently produced samples and target number of samples. The progress bar allows early stopping. The simulation entries have buttons to add more samples and buttons to generate their associated visualizations. The visualization buttons bring up a dialog to set the parameters of the visualization. After generating a visualization, it is displayed for the user and they can decide whether to save it (bringing up a dialog to select where) or discard it.

There is also a list of workers for a project. Each worker has a number of hours they complete per workday and a name. A button on the DAG button bar opens a worker list editor.
...
A sampling run can fail. If all workers are available and no tasks are running and there are unfulfilled dependencies for a task in the current possible world, that sampling run has failed. Save the final state and mark it as failed - allow the user to see what tasks could not be completed. More than likely, there are too many worker exclusions for the workers that could be assigned. The fraction of failed runs should be on the main simulation list display.
